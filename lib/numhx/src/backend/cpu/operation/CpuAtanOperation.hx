 ...
 * @author leonaci
 */
class CpuAtanOperation extends CpuNdArrayOperation
{
	...
	override public function run():Void {
		...
		if (dst.naive) dst.buffer.setValue([for (i in 0...dst.size) dst.buffer.set(i, Math.atan(v[i]))]);
		else {
			var ndim = dst.ndim;
			var shape = dst.shape;
			var size = dst.size;
			...
			for (i in 0...size) {
				...
				dst.buffer.set(idx, Math.atan(v[i]));
			}
		}
	}
}