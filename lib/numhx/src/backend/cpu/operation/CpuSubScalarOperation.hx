 ...
 * @author leonaci
 */
class CpuSubScalarOperation extends CpuNdArrayOperation
{
	...
	override public function run():Void {
		...
		if (dst.naive) dst.buffer.setValue([for (i in 0...dst.size) MathUtil.sub(v[i], b)]);
		else {
			var ndim = dst.ndim;
			var shape = dst.shape;
			var size = dst.size;
			...
			for (i in 0...size) {
				...
				dst.buffer.set(idx, MathUtil.sub(v[i], b));
			}
		}
	}
}